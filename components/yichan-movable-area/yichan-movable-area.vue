	...
	<view>
		<view class="slider-container">
			...
			<movable-area class="sliderBar" :style="{width: (100)+'%'}" :damping="10">
				<view class="gone" :style="{width: x +'px', 'transition' : `width ${delay}s ease`}"></view>
				<movable-view class="slider" direction="all" :x="x" @change="onChange" :disabled="disabled">
					<text>{{ score }}</text>
				</movable-view>
			</movable-area>
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			updateScore(score) {
				if (['string', 'number'].includes(typeof score !== 'string')) {
					throw ('数据格式错误')
					return
				}
				...
				this.x = ((score - this.minScore) / (this.maxScore - this.minScore)) *
					this.slideBarWidth;
				this.$nextTick(() => {
					this.score = score
				})
